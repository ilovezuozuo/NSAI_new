...
import sys
sys.path.append("/src/Projrct/LSTM_for_insert")
from LSTM_for_insert.pose_predictor import MyModelPredictor
...
class TrueInsert(TrueBase):
    def __init__(self, group_):
        ...
        self.model_predictor = MyModelPredictor('~/home/inexbot/NSAI_new/src/Projrct/LSTM_for_insert/lstm_model_0221.pth', 
                                                '~/home/inexbot/NSAI_new/src/Projrct/LSTM_for_insert/data_mean_0221.npy', 
                                                '~/home/inexbot/NSAI_new/src/Projrct/LSTM_for_insert/data_std_0221.npy')
        self.stop = False
        self.fail = False
        self.data = []
    ...
    # 这个函数的作用是计算从当前执行器z轴移动到理想的预测z轴（螺栓z反向）的最短的旋转方向
    ...
    def action(self, all_info, pre_result_dict,kalman,yolo,plc,sleeve_type):
        # 这里是使用mmlab的架构直接实例化了一个mobilevit
        self.inferencer = ImageClassificationInferencer(model='~/home/inexbot/NSAI_new/src/Projrct/VIT/VIT_for_insert/config.py', pretrained='~/home/inexbot/NSAI_new/src/Projrct/VIT/VIT_for_insert/epoch_91.pth', device='cuda')

        # 围绕着输入粗位置先使用get_insert_pose随机产生一个insert_pose
        # 粗位置true_x,true_y,true_z来源于卡尔曼的估计结果
        ...
        pose=kalman.get_former_pose()

        true_x,true_y,true_z = pose.position.x, pose.position.y, pose.position.z
        # true_x,true_y,true_z = 0.3712,0.0838,0.0744
        print("start to adjust")

        # 这里是对一个有可能直接套上的位置加了随机化，使得误差大一些，进行姿态估计，实际部署的时候为了快一点尚可优化
        ...
        # 初始化一个计数器，在100次内LSTM肯定能给出一个mobilevit认可的结果（阈值判断法）。
        # 注意，这里是滑动时间窗截取信息输入LSTM进行预测，输入是序列，输出只取最后一个结果
        t = 0
        while (not self.stop) and (t < 100):
            # print (len(planner.wrench_list))               
            sample = np.array(planner.wrench_list[-15:])
            ft = sample[:,8:14]
            self.plot(ft) # 绘制雷达图，mobilevit根据雷达图检验是否完成
            result = self.inferencer('radar.jpg')[0]
            tool_pose = sample[-1, 1:8]

            # 使用LSTM预测相对位姿并计算世界坐标系下的绝对位姿
        ...
        # 上面的代码只是适应了最佳的法向量，具体的还要有老版本的insert的摸索过程（以下部分）
        plc.set_effector_star_pos(200)
        rospy.sleep(0.2)
        planner.collect = True #记录力的开关打开
        s = 0
        t = 0
