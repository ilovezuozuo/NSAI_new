...
class MLP_self(nn.Module):
    ...
    def __init__(self, num_i, num_h, num_o, dropout_prob=0.04):
...
def check_collision(rect1_center, rect1_size, rect2_base_center, rect2_size):
    # rect1_center: 长方形1中心位姿 [x, y]
    # rect1_size: 长方形1长宽 [length, width]
    # rect2_base_center: 长方形2底盘中心位姿 [x2, y2, 0]
    # rect2_size: 长方形2长宽 [length, width]

    # 将所有输入转换为PyTorch张量
    rect1_center = torch.tensor(rect1_center)
    rect1_size = torch.tensor(rect1_size)
    rect2_base_center = torch.tensor(rect2_base_center)
    rect2_size = torch.tensor(rect2_size)

    # 计算两个长方形的半尺寸
    rect1_half_size = rect1_size / 2
    rect2_half_size = rect2_size / 2

    # 计算两个长方形的边界
    rect1_min = rect1_center - rect1_half_size
    rect1_max = rect1_center + rect1_half_size
    rect2_min = rect2_base_center - rect2_half_size
    rect2_max = rect2_base_center + rect2_half_size

    # 判断是否发生碰撞
    collision_x = (rect1_min[0] <= rect2_max[0]) and (rect1_max[0] >= rect2_min[0])
    collision_y = (rect1_min[1] <= rect2_max[1]) and (rect1_max[1] >= rect2_min[1])
...
def calculate_IK_loss(angle_solution,aaaaaaaaaa, bbbbbbbbbb):
    ...
    return IK_loss
def collision_free_loss(input, output):
    relu = nn.ReLU(inplace=True)
    # print("collision_free_loss_input:", output[3:6])
    inputposition = input[3:6]
    outputposition = output[3:6]
    distance = torch.sqrt(torch.sum(torch.square(inputposition - outputposition)))
    print("distance", distance)
    if distance >= 0.68:
        loss = (distance - torch.tensor([0.6])) * 2000

    elif 0.5 < distance < 0.68 :
        loss = relu(0.3 - distance)
        global num_distance_right
        if abs(input[4]-output[4])> 0.50:
            if  abs(input[3]-output[3])< 0.15:
                print("look at here!!!!!!!!!!!!!!!!!!!!!!!!!!!!",abs(input[3]-output[3]))
                num_distance_right = num_distance_right + 1
    else:
        loss = (torch.tensor([0.3]) - distance) * 2000
    return loss
...
class TrueMove(TrueBase):
    ...
    def action(self, all_info, pre_result_dict, kalman,yolo):
        ...
        distance = torch.sqrt((torch.sum(torch.square(torch.tensor([target.position.x, target.position.y])))))
        if distance <= 0.65:
            pass
        else:
            ...
            filename = "~/home/inexbot/NSAI_new/src/Projrct/RPSN/checkpoint-epoch800.pt"
            filename2 = "~/home/inexbot/NSAI_new/src/Projrct/RPSN/checkpoint-epoch8001000.pt"
            filename3 = "~/home/inexbot/NSAI_new/src/Projrct/RPSN/mir_ur/checkpoint-epoch8001600.pt"
            ...
            global result
            num_sample_tochange_model = 0
            # result = []
            while True:
                num_sample_tochange_model = num_sample_tochange_model+1
                # print('data_loader_test',data_loader_test)
                print('num_NOError2',num_NOError2)
                print('num_distance_right',num_distance_right)

                for data in data_loader_test:
                    # with torch.no_grad():
                        inputs = data[0]
                        inputs[0][3] =  inputs[0][3]-15.
                        inputs[0][4] = inputs[0][4] - 3.
                        # print('data_loader_test',data_loader_test)
                        print('inputs:', inputs)
                        ...
                        rect1_center = [18.6, 5.0]
                        rect1_size = [1.85, 1.15]
                        rect2_base_center = [outputs[0][3]+15., outputs[0][4]+3.]
                        rect2_size = [0.8, 0.8]
                        print("outputs:", outputs)

                        collision = check_collision(rect1_center, rect1_size, rect2_base_center, rect2_size)
                        print("是否发生碰撞:", collision)
                        while collision:
                            intermediate_outputs = model(inputs)
                        # print('intermediate_outputs:', intermediate_outputs)
                            input_tar = shaping(inputs)
                            ...
                            collision = check_collision(rect1_center, rect1_size, rect2_base_center, rect2_size)
                            print("是否发生碰撞:", collision)
                            print("RPSN output is:", [outputs[0][3]+15., outputs[0][4]+3.])
                            if not collision:
                                print("bu peng zhuang:", collision)
                                break
                        ...
                        for i in range(len(inputs)):
                            ...
                            + collision_free_loss(inputs[i], outputs[i])
                if  num_NOError2 == len(data_loader_test) and num_distance_right == len(data_loader_test):
                    break
                else:
                    num_NOError2= 0
                    num_distance_right= 0
                if num_sample_tochange_model == 50:
                    checkpoint = torch.load(filename2)
                    model.load_state_dict(checkpoint['state_dict'])
                if num_sample_tochange_model == 100:
                    checkpoint = torch.load(filename3)
                    model.load_state_dict(checkpoint['state_dict'])
                if num_sample_tochange_model == 150:
                    print("all models tried, 150times,no solution!")
                    break
            ...
            print("final_result_of_joint_angle", final_result)
            # print(len(outputs_of_MLP))
            # rospy.sleep(10000)
            print("finally go to:", outputs[0][3]+15., outputs[0][4]+3., outputs[0][2])
            if (outputs[0][4]+3)>5:
                mir.MIR_move(outputs[0][3]+15., outputs[0][4]+3., outputs[0][2]+180)
                print(outputs[0][2],"xuanzhuan180du!!!!!!!!!!!!!!!!!!!!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
            else:
                mir.MIR_move(outputs[0][3]+15., outputs[0][4]+3., outputs[0][2])
                print(outputs[0][2],"rz is !!!!!!!!!!!!!!!!!!!!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")

        target = self.transform_pose("map",
                                    "base_footprint",
                                    tgt_pose_in_map,
                                    rospy.Time(0) )
        target.position.z = target.position.z+0.03
